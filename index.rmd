...
When I first learned about utility functions and indifference curves, I was shown an intimidating figure that looked a bit like the image below. If you were lucky, you were shown a computer generated image. The less fortunate had a professor furiously scribbling them onto a board.
<br>
<br>
...
Simply stated, a utility function is a function that explains the amount of utility a consumer posesses given their consumption of two different goods, $x$, and $y$. An indifference curve is just an infintesimal slice that function which describes all the different combinations between two goods that yields the same amount of utility (i.e. to which a person would be indifferent). So, an indifference function defined by $U(x,y) = 2x + y$ would mean that a person would be have an indifference to two "$x$'s" or one "$y$" and all the differernt possibilities along that curve. In more practical terms, let x represent quantity of wine while y represents a quantity of cheese. If the same  utility function from above applies, then a consumer would be indifferent between two units of wine and one serving of cheese.
<br>

An indifference curve in two dimensions could look something like this:
<br>
...
1. Notice that the curve slopes downwards - this must necessarily be the case; as one increases their consumption of $y$, they will give up the other good that they were indifferent to, $x$.

2. Everything below the curve represents bundles that do not maximize utility (the sums of both $x$ and $y$ do not yield a utility equal to those along the curve). Utility theory assumes that a consume will always seek to maximize utility.

3. Understand that the slope is non-linear. In general, the more one has of something, the less utility they will gain from another and conversely, the more they would give up to acquire the other good. This slope has an official name: Marginal Rate of Substitution or $MRS$. We will come back to that later.  
<br>

But that was just one slice that I have already noted as infinitesimally small.
<br>
...
Manipulate the graphs and take a look around. Here we can see that all of those two dimensional lines are really just <span style="color:#59b300">**projections of a much larger 3-dimensional utility function**</span>. And the true indifference curve that a consumer is to is only limited by the amount of utility they can purchase.
<br>
...
As I said earlier, utility theory suggests that one will seek to maximize their utility in all situations. So, naturally, their must be some constraint else a consumer would consume indefinitely. And so it isn't hard to imagine that the consumer's constraint is often a set budget - a finite amount of money they have to spend. A budget, then, tells us how much we can spend and therefore we must <span style="color:#59b300">**maximize our utility within that constraint**</span>. 
<br>

Let us use $z$ again to denote our budget and assume that wine is half the amount of cheese. In this case, the price of one unit of wine, $P_x$, is \$0.25 and the price of one unit of cheese, $P_y$, is \$0.50 (we are living in a golden age). Your budget is then defined as $$I \leq (1/4)x + (1/2)y$$.

The coefficients of a budget function represent prices and could be anything in theory. In this case, I can afford 2 units of cheese to one unit of wine and my budget constraint, $I$, will tell me how much of each I can buy. Additionally, the ratio of prices, $P_y/P_x$, represents the slope of the budget function.
<br>
...
As your budget increases, $z$, so does your ability to purchase larger combinations of goods, $x$ and $y$. But a consumer is limited to the budget she has. And, similar to the consumers utility: we assume that a consumer maximizes their budget (so that $budget = (1/4)x + (1/2)y$) and that everything below the curve is considered suboptimal. As a persons budget increases, the line shifts right, and left as a budget decreases.
<br>
...
Finally, knowing our $x$ and $y$ values, we can see that our budget is maximized and the maximum utility we can purchase is around 77 units!
...
$$\mathcal{L}(x, y, \lambda) = U(x,y) + \lambda(I - xP_x + yP_y) $$
...
$$\frac{\partial \mathcal{L}}{\partial x} = .6x^{-.4}y^.4 - \frac{\lambda}{4} = 0$$
$$\frac{\partial \mathcal{L}}{\partial y} = x^{.6}.4y^{-.6} - \frac{\lambda}{2} = 0$$
$$\frac{\partial \mathcal{L}}{\partial \lambda} = 50 - (1/4)x - (1/2)y = 0$$
<br>

Solving for these systems of equations, we get the same result as above
$$ y = 40, x = 120$$

Although, one interesting thing that comes from solving the utility maximization problem this way, is that we can extract the value of $\lambda$. This value represents what is known as <span style="color:#59b300">**the shadow price**</span> of the budget constraint, which is the incremental value, in terms of one's utility, of adding one additional dollar to your budget. Since we know $x$ and $y$ here, we can calculate;

$$.6x^{-.4}y^.4 = \frac{\lambda}{4}$$
